...
<div>
            ...
            <li v-for="item in cartList">
              <div class="cart-tab-1">
                <div class="cart-item-check">
                  <a class="checkbox-btn item-check-btn" v-bind:class="{'check':item.checked==true}" @click="editCart('checked',item)">
              ...
              <div class="cart-tab-5">
                <div class="cart-item-opration">
                  <a href="javascript:;" class="item-edit-btn" @click="deleteIs(item.productId)">
  ...
  <modal v-bind:mdShow="deleteShow" v-on:close="closeModal">
    <p slot="message">
      <span>确认删除么</span>
    </p>
    <div slot="btnGroup">
      <a class="btn btn--m" @click="deleteCart">确认</a>
      <a class="btn btn--m" @click="deleteShow = false">关闭</a>
    </div>
  </modal>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toggleCheckedAll(){
        this.checkedAllFlag = !this.checkedAll;
        this.cartList.forEach((item) =>{
          item.checked = this.checkedAllFlag;
        });
        axios.post("/users/editCheckAll",{checkedAllFlag:this.checkedAllFlag}).then((response)=>{
          let res = response.data;
          if(res.status == '0'){
