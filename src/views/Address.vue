...
<div>
    ...
    <div class="container">
      <div class="checkout-addr">
        ...
        <div class="addr-list-wrap">
          <div class="addr-list">
            <ul>
              <li v-for="(item,index) in addressListLimit" v-bind:class="{'check':indexChecked==index}" @click="indexChecked=index;selectedAddrId=item.addressId">
                <dl>
                  <dt>{{item.userName}}</dt>
                ...
                <div class="addr-opration addr-del">
                  <a href="javascript:;" class="addr-del-btn" @click="isDeleteAddress(item.addressId)">
  ...
  <modal v-bind:mdShow="isDeleteAddr" @close="closeModal">
    <p slot="message">是否删除此地址</p>
    <div slot="btnGroup">
      <a class="btn btn--m" href="javaScript:;" @click="deleteAddress">确定</a>
      <a class="btn btn--m" href="javaScript:;" @click="isDeleteAddr=false">取消</a>
    </div>
  </modal>
  ...
  import axios from 'axios'
    export default {
      ...
      methods:{
        ...
        isDeleteAddress(addressId){
          this.addressId = addressId;
          this.isDeleteAddr = true
        },
        deleteAddress(){
          axios.post("/users/deleteAddress",{addressId:this.addressId}).then((response)=>{
            let res = response.data;
            if(res.status == '0'){
